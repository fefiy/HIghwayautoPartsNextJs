...
const ClientPage = ({  brands, isFetched}: { brands: IOptional[], isFetched?:boolean }) => {
  let pending =  true
 ...
 if(pending && !isFetched){
  ...
  return (
    <section className="bg-white w-full min-h-[90%] flex flex-col ">
      {openAddCategory && <AddBrandModal closeModal={closeModal} />}
      ...
      <Button text="Add Brand" openModal={openModal} />
      { brands.length > 0 ? (
